...
#include <iostream>
    ...
    namespace ECS::Systems
    {
                ...
                for (auto & componentPool : componentPools) {
                    for (auto & componentName : _componentNames1) {
                        if (componentPool->getComponentName() == componentName) {
                            if (i < sizeof...(T1s)) {
                                newComponentPoolsArray1[i] = componentPool;
                            }
                            i++;
                        }
                    }
                    for (auto & componentName : _componentNames2) {
                        if (componentPool->getComponentName() == componentName) {
                            if (j < sizeof...(T2s)) {
                                newComponentPoolsArray2[j] = componentPool;
                            }
                            j++;
                        }
                    }
                    ...
                    for (size_t j = 0; j < _componentPoolsArrays2.size(); j++) {
                        ...
                        for (size_t k = 0; k < chunkCount1; k++) {
                            for (size_t l = 0; l < chunkCount2; l++) {
                                _run(_componentPoolsArrays1[i], _componentPoolsArrays2[j], k, l);
                            }
                            std::cout << "\nnew chunk iteration\n";
                        }
                    }
        ...
        protected:
            void _run(std::array<Components::IComponentPool *, sizeof...(T1s)>& componentPools1, std::array<Components::IComponentPool *, sizeof...(T2s)>& componentPools2, size_t index1, size_t index2) {
                auto componentPoolsTuple1 = std::apply([index1](auto&... pools) {
                    return std::make_tuple(dynamic_cast<T1s*>(pools)->getRawStdVectors(index1)...);
                }, componentPools1);
                auto componentPoolsTuple2 = std::apply([index2](auto&... pools) {
                    return std::make_tuple(dynamic_cast<T2s*>(pools)->getRawStdVectors(index2)...);
                }, componentPools2);

                std::apply([this, &componentPoolsTuple1, &componentPoolsTuple2](auto &... componentPools1) {
                    std::apply([this, &componentPools1...](auto &... componentPools2) {
                        _operate(componentPools1..., componentPools2...);
            ...
            inline void _operate(typename T1s::VTypes&... componentVectors1, typename T2s::VTypes&... componentVectors2) {
                ...
                for (size_t i = 0; i < vectorSize1; ++i) {
                    for (size_t j = 0; j < vectorSize2; ++j) {
                        ...
                        _callInnerOperate(refTuple1, refTuple2, std::make_index_sequence<std::tuple_size<decltype(refTuple1)>::value>{});
                    }
                    std::cout << " ";
                }
                std::cout << "\n";
            ...
            virtual inline void _innerOperate(typename T1s::Types&... componentPools1, typename T2s::Types&... componentPools2) = 0;
