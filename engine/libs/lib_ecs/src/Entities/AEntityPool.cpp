...
namespace E {
  ...
  std::vector<Chunks::ChunkPos> &AEntityPool::getFreePos() { return _freePos; }

  Chunks::chunk_pos_t AEntityPool::getTotalSize() { return _entityStatusPool.elemCount(); }

  Chunks::chunk_pos_t AEntityPool::getUsedSize() { return _entityStatusPool.elemCount() - _freePos.size(); }

  Chunks::chunk_pos_t AEntityPool::getChunkCount() const { return _entityStatusPool.chunkCount(); }

  C::IComponentPool *AEntityPool::getComponentPool(const std::string &name)
  {
    for (auto *componentPool : getComponentPools()) {
      if (componentPool->getComponentName() == name) { return componentPool; }
    }
    return nullptr;
  }
  ...
  void AEntityPool::addChunk()
  {
    ...
    for (auto *componentPool : getComponentPools()) { componentPool->addChunk(_chunkSize); }
