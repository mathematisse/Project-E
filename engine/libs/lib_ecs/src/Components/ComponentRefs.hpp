...
public:
   ...
   */
  explicit ComponentRef(Ts *...components) : _components(std::make_tuple(components...)) {}
   ...
   */
  explicit ComponentRef(Ts... components) : _components(createPointerTuple(components...)) {}
   ...
   */
  template<std::size_t Index> decltype(auto) get() { return std::get<Index>(_components); }
   ...
   * @return Pointer to the component value.
   */
  template<std::size_t Index> decltype(auto) get() const { return std::get<Index>(_components); }
   ...
   */
  template<std::size_t Index> void set(const std::tuple_element_t<Index, std::tuple<Ts...>> &value)
  {
