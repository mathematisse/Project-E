...
template<typename... Args> std::string concatenate(Args... args) { return (toString(args) + ...); }
...
inline boost::log::trivial::severity_level getSeverityLevelFromEnv()
{
  ...
  if (logLevelStr == "trace") { return boost::log::trivial::trace; }
  if (logLevelStr == "debug") { return boost::log::trivial::debug; }
  if (logLevelStr == "info") { return boost::log::trivial::info; }
  if (logLevelStr == "warning") { return boost::log::trivial::warning; }
  if (logLevelStr == "error") { return boost::log::trivial::error; }
  if (logLevelStr == "fatal") { return boost::log::trivial::fatal; }
...
inline void init()
{
  ...
  boost::log::add_file_log(boost::log::keywords::file_name = "app_%N.log",
...
template<typename... Args> void trace(Args... args) { BOOST_LOG_TRIVIAL(trace) << concatenate(args...); }

template<typename... Args> void debug(Args... args) { BOOST_LOG_TRIVIAL(debug) << concatenate(args...); }

template<typename... Args> void info(Args... args) { BOOST_LOG_TRIVIAL(info) << concatenate(args...); }

template<typename... Args> void warn(Args... args) { BOOST_LOG_TRIVIAL(warning) << concatenate(args...); }

template<typename... Args> void error(Args... args) { BOOST_LOG_TRIVIAL(error) << concatenate(args...); }

template<typename... Args> void fatal(Args... args) { BOOST_LOG_TRIVIAL(fatal) << concatenate(args...); }
