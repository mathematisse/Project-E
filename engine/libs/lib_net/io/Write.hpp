...
public:
    ...
    virtual Result<size_t> write(const std::span<std::uint8_t> &buf) = 0;
    ...
    virtual Result<size_t> write_vectored(const std::span<std::span<std::uint8_t>> &bufs);

    // Determines if this writer has an efficient write_vectored implementation.
    [[nodiscard]] virtual bool is_write_vectored() const { return false; }

    // Attempts to write an entire buffer into this writer.
    Result<result::Void> write_all(const std::span<std::uint8_t> &buf);

    // Attempts to write multiple buffers into this writer.
    Result<result::Void> write_all_vectored(std::span<std::span<std::uint8_t>> &bufs);
...
template<typename T>
concept Writable = requires(T t, const std::span<std::uint8_t> &buf) {
